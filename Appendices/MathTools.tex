...
%
\begin{definition}{\textbf{Hash function - syntax}}~\textnormal{\cite{Katz:2007:IMC:1206501}}
...
\begin{definition}~\textnormal{\cite{Katz:2007:IMC:1206501}}
  A hash function $H$ is \textbf{collision resistant} if for all probabilistic polynomial-time adversaries $\mathcal{A}$ there exists a negligible function $\textbf{negl}(\cdot)$ such that
...
More specifically, the hashes of the items consist the leafs of the tree and the last level. The internal levels are defined recursively as follows: To create level $k-1$ each pair of level $k$, $(A, B)$, is transformed as a node of value $H(A || B)$ which points to both $A$ and $B$ for some hash function $H(\cdot)$. If the number of nodes at level $k$ is odd, the last node at that level is paired with itself\footnote{This specific construction is the one Bitcoin implements. There are various other constructions which are beyond the scope of our work.}.
\vspace{0.3cm}
\begin{figure}[H]
  \centering
...
\begin{definition}{\textbf{Oracle Indistinguishability}}~\textnormal{\cite{ACI}}
...
\begin{definition}{\textbf{Pseudorandom Function}}~\textnormal{\cite{ACI}}
...
\begin{definition}{\textbf{Security model }}
...
\begin{definition}{\textbf{Directed Acyclic Graph}}~\textnormal{\cite{ForlerLW13}}
...
\begin{definition}{\textbf{Parallel/Sequential Graph Pebbling}}~\textnormal{\cite{cryptoeprint:2016:875}}
...
\end{definition}
%
\begin{definition}{\textbf{Time/Space/Cumulative Pebbling Complexity}}~\textnormal{\cite{cryptoeprint:2016:875}}
