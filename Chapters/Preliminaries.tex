...
\begin{enumerate}
  ...
  \item \label{Condition 2.} Computing $PS(pwd)$ should be "as slow as possible", without contradicting Condition~\ref{Condition 1.}.
...
The above definition, although extremely rigid, is not that intuitive. In order to describe memory requirements, we can describe the same notion we will mention another definition given in \cite{ForlerLW13}, without causing any conflict with definition \ref{Memhard1}. Before we give the second definition, one should notice that for any parallelized attack, using $b$ cores, the required memory per core is decreased by a factor of $\frac{1}{b}$, and vice versa.
